/**
 * SFN 캠페인 생성 페이지 — Phase 4-B (앵커 통합 후).
 *
...
export default function SfnCampaignCreatePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitError('');
    setSubmitting(true);
    const { data, error } = await sfnCampaignCreateAPI.create({
      name: name.trim(),
    ...
    if (error) {
      ...
      if (data?.id) {
        ...
        router.push(`/sfn/campaigns/${data.id}`);
      }
      return;
    }
    if (data?.id) {
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <MainCard>
          <div className="space-y-4">
            <div>
              <h1 className="text-lg font-semibold">새 SFN 캠페인 생성</h1>
