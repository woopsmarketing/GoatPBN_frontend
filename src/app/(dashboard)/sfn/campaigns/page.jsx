/**
 * SFN (Next.js PBN) 캠페인 목록 페이지 — Phase 4 격리 개발 버전.
 *
 * 운영 코드 미수정 (CLAUDE.md 격리 개발 원칙). 검증 후 통합 위치로 승격.
 *
 * 기능 (최소):
 *  - sfn_campaigns 리스트 카드 표시
 *  - active ⇌ paused 토글
...
import { sfnCampaignsAPI } from '../../../../features/sfn/api';
...
async function fetchCampaigns() {
  const { data, error } = await sfnCampaignsAPI.list();
  if (error) throw new Error(error.message || '캠페인 목록을 불러오지 못했습니다.');
  return data || [];
}

export default function SfnCampaignsPage() {
  ...
  const {
    ...
    isValidating,
    mutate
  } = useSWR('/sfn/campaigns', fetchCampaigns, {
  ...
  const handleToggle = async (campaign, nextStatus) => {
    ...
    const { data, error } = await sfnCampaignsAPI.setStatus(campaign.id, nextStatus);
    setTogglingId(null);
    if (error) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">SFN 파이프라인 기반 자동 발행 캠페인 관리 (격리 개발 버전)</p>
        </div>
        <div className="flex items-center gap-2">
          <TailwindButton variant="primary" size="sm" onClick={() => router.push('/sfn/campaigns/create')}>
            + 새 캠페인
          </TailwindButton>
      ...
      {loading && campaigns.length === 0 ? (
        ...
        <MainCard>
          <div className="py-10 text-center text-gray-500">
            <p className="text-base">아직 생성된 SFN 캠페인이 없습니다.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {campaigns.map((c) => (
            <CampaignCard
              ...
              onOpen={(camp) => router.push(`/sfn/campaigns/${camp.id}`)}
            />
          ))}
        </div>
      )}
