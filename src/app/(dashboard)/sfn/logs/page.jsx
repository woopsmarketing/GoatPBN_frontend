...
import { sfnJobsAPI } from '../../../../features/sfn/api';
...
async function fetchJobs() {
  const { data, error } = await sfnJobsAPI.listAllForUser({ limit: 200 });
  if (error) throw new Error(error.message || '잡 목록을 불러오지 못했습니다.');
  return data || [];
}

export default function SfnLogsPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">SFN 로그</h1>
                    ...
                    return (
                      <tr key={j.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 pr-2">
                          <button
                            type="button"
                            onClick={() => router.push(`/sfn/campaigns/${j.campaign_id}`)}
