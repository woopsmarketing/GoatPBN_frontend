...
export default function UsagePage() {
  ...
  useEffect(() => {
    ...
    const loadUsage = async () => {
      try {
        ...
        if (!user) {
          setError(isEnglish ? 'Could not find login info.' : '로그인 정보를 찾을 수 없습니다.');
          return;
        }
        ...
        if (active) setError(isEnglish ? 'Failed to load usage data.' : '사용량 데이터를 불러오지 못했습니다.');
      } finally {
  ...
  return (
    <div className="space-y-6">
      <MainCard title={isEnglish ? 'Credit usage summary' : '크레딧 사용 요약'}>
        {loading ? (
          <p className="text-sm text-gray-600">{isEnglish ? 'Loading usage data...' : '사용량 데이터를 불러오는 중입니다...'}</p>
        ) : error ? (
          ...
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-lg bg-rose-50 p-4 text-center">
                <p className="text-xs font-medium text-rose-700 uppercase tracking-wide">{isEnglish ? 'Failed attempts' : '실패한 시도'}</p>
            ...
            <div className="rounded-lg border border-gray-200 p-4">
              <h3 className="text-sm font-semibold text-gray-900">{isEnglish ? '📆 Daily usage' : '📆 일자별 사용량'}</h3>
              <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                {Object.keys(dailyUsage).length === 0 && (
                  <p className="text-sm text-gray-500">
                    {isEnglish ? 'No content has been generated yet.' : '아직 생성된 콘텐츠가 없습니다.'}
                  </p>
                )}
      ...
      <MainCard title={isEnglish ? 'Recent usage' : '최근 사용 내역'}>
        {loading ? (
          <p className="text-sm text-gray-600">{isEnglish ? 'Loading recent usage...' : '최근 사용 내역을 불러오는 중입니다...'}</p>
        ) : logs.length === 0 ? (
          <p className="text-sm text-gray-600">
            {isEnglish
              ? 'No usage history yet. Once you generate content, it will appear here.'
              : '아직 기록된 사용 내역이 없습니다. 캠페인을 생성하면 자동으로 기록됩니다.'}
          </p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-2 text-left font-medium text-gray-500">{isEnglish ? 'Created at' : '생성일'}</th>
                  <th className="px-4 py-2 text-left font-medium text-gray-500">{isEnglish ? 'Keyword' : '키워드'}</th>
                  <th className="px-4 py-2 text-left font-medium text-gray-500">{isEnglish ? 'Status' : '상태'}</th>
                  <th className="px-4 py-2 text-right font-medium text-gray-500">{isEnglish ? 'Credits used' : '사용 크레딧'}</th>
                  <th className="px-4 py-2 text-left font-medium text-gray-500">{isEnglish ? 'Post URL' : '게시물 URL'}</th>
                </tr>
              </thead>
                ...
                {logs.map((log) => (
                  <tr key={log.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">
                      ...
                      >
                        {log.status === 'success' ? (isEnglish ? 'Success' : '성공') : isEnglish ? 'Failed' : '실패'}
                      </span>
                    </td>
